...
void fillCircle(Bmp* this, Color c, int x, int y, int radius) {
	...
	/*walks through the average of the circle on y achses*/
	for (i = -radius; i <= radius; i++) {
		/*walks through the average of the circle on x achses*/
		for (j = -radius; j <= radius; j++) {
			/*if coordinate is in the circle (not on the outside)*/
...
Bmp* barChart(double values[], int valueLength) {
	...
	/*variable width, depends on how many bars are needed*/
	const int bmpWidth = valueLength * barWidth + (valueLength - 1) * padding + 2 * margin;
	...
	/*look up biggest values (y achses)*/
	for (i = 1; i < valueLength; i++) {
		if (values[i] > biggestValue) {
			biggestValue = values[i];
		}
	}
	/*for every value on array: fill rectangle with calculatet size and different colors*/
	for (i = 0; i < valueLength; i++) {
		newBarHeight = (bmpHeight - 2 * margin) * (values[i] / biggestValue);
		fillRect(this, colors[colorIndex++], (margin + (barWidth + padding) * i), margin, barWidth, newBarHeight);
		/*set colorIndex back if the end is reached*/
		if (colorIndex == sizeof(colors) / sizeof(Color)) {
			colorIndex = 0;
		}
	}
 ...
 *
 */
void drawPie(Bmp* this, Color c, int x, int y, int radius, double startPie, double endPie) {
	...
	/*b=pi*r²*(Alpha/360)*/
	/*draw pie degree fpr degree*/
	for (r = radius; r > 0; r--) {
		for (alpha = startPie * (1 + radius / 10); alpha < endPie * (1 + radius / 10); alpha++) {
...
Bmp* pieChart(double values[], int valueLength) {
	...
	/*the first pie starts at 0.0*/
	startPie = 0.0;
	endPie = 0.0;

	/*draw pie of every value in values[]*/
	for (i = 0; i < valueLength; i++) {
		/*calculates where the pie should end*/
		endPie = startPie + (values[i] / sum) * 360;
		drawPie(this, colors[colorIndex++], 500, 500, 400, startPie, endPie);
		/*end of the last pie is start of the next one*/
		startPie = endPie;
		/*set back to keep in range*/
		if (colorIndex >= sizeof(colors) / sizeof(Color)) {
			colorIndex = 0;
		}
	}
